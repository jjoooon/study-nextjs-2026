...
1. 모든 질문과 대답은 한글로 작성한다.
2. 사용자에게 선택지(모달 질문 옵션 등)를 제시할 때는 질문과 모든 선택 항목을 **항상 한국어로 작성**해야 한다.
3. 선택지 클릭은 필수사항이 아니며, 사용자가 모달 하단의 직접 입력(Write-in)란에 한국어로 자유롭게 답변하거나 건너뛸 수 있도록 안내한다.
