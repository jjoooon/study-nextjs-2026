...
- routed 팝업은 실제 페이지 로드라서 `Providers`가 다시 실행되며 자기만의 Redux store/`globalRegistry`/`DialogRoot`를 새로 만든다. 그 iframe **안에서** `popup.open()`/`confirm()`/`alert()`를 호출하면 원래는 그 iframe 자신의 로컬 `DialogRoot`가 렌더링해서 iframe 박스 크기로 클리핑된다 — 이를 막기 위해 `popupApi.ts`의 `open()`이 `isInIframe()`이면 로컬 store에 dispatch하는 대신 `window.top`으로 `postMessage` relay를 보내고, top 쪽에 상시 마운트된 `PopupRelayHost`가 그 요청을 받아 **자기 자신의 `popup.open()`을 그대로 재호출**한다(새 상태관리 로직 없이 기존 함수 재사용). 결과는 relay 응답으로 다시 내려보낸다. 이러면 서브 팝업은 항상 top의 진짜 `DialogRoot`가 그리므로 클리핑이 없고, `dialogOverlayRegistry.ts`도 top 하나로 통일되어 딤(overlay) 순서도 자동으로 맞는다.
...
### Task 7: 서브 팝업 `window.top` relay (중첩 팝업 클리핑 해결)

**Files:**
- Modify: `src/shared/utils/popup/popupBridge.ts`
- Modify: `src/shared/utils/popup/popupApi.ts`
- Create: `src/shared/components/popups/PopupRelayHost.tsx`
- Modify: `src/app/layout.tsx`

**Step 1: `popupBridge.ts`에 relay 메시지 타입 + `isInIframe`/`relayOpenToTop` 추가**

Task 1에서 만든 파일 전체를 아래로 교체:
...
export type PopupBridgeMessage =
  ...
  | { channel: typeof POPUP_BRIDGE_CHANNEL; type: 'reject'; id: string; message: string }
  | {
      channel: typeof POPUP_BRIDGE_CHANNEL;
      type: 'relayOpen';
      id: string;
      popupType: string;
      props: Record<string, unknown>;
    }
  | { channel: typeof POPUP_BRIDGE_CHANNEL; type: 'relayResolve'; id: string; result?: unknown }
  | { channel: typeof POPUP_BRIDGE_CHANNEL; type: 'relayReject'; id: string; message: string };
...
/**
 * 현재 window가 iframe 안에서 실행 중인지 여부.
 *
 * @description
 * routed 팝업(/popup/{type})은 실제 페이지 로드라서 자기만의 Redux store/DialogRoot를
 * 새로 만든다. 그 안에서 popup.open()으로 서브 팝업을 열면 이 iframe 자신의 DialogRoot가
 * 렌더링하게 되어 iframe 박스 크기로 클리핑된다. isInIframe()이 true면 popupApi.open()이
 * 로컬 store 대신 window.top으로 relay한다.
 */
...
// ============================================================================
// RELAY (서브 팝업을 window.top에서 열리게 중계)
// ============================================================================
...
function ensureRelayListenerAttached() {
  if (relayListenerAttached || typeof window === 'undefined') return;
  ...
  window.addEventListener('message', (event: MessageEvent) => {
    ...
    const data = event.data;
    if (!isPopupBridgeMessage(data)) return;

    const pending = pendingRelays.get(data.id);
    if (!pending) return;

    if (data.type === 'relayResolve') {
      pendingRelays.delete(data.id);
      pending.resolve(data.result);
    } else if (data.type === 'relayReject') {
      pendingRelays.delete(data.id);
...
/**
 * iframe 안에서 popup.open()이 호출됐을 때, window.top에 팝업을 대신 열어달라고
 * 요청하고 그 결과를 돌려받는다. window.parent가 아니라 항상 window.top으로 직행한다 -
 * routed 팝업의 iframe은 항상 top 페이지의 DialogRoot가 직접 자식으로 만들기 때문에
 * 실제로 iframe-in-iframe 중첩은 생기지 않는다 (몇 단계든 한 홉으로 끝남).
 *
 * @param id  이 relay 요청/응답을 짝짓는 상관관계 id. Redux에 등록되는 실제 팝업 id와는
 *            별개 - 그 id는 window.top의 popup.open()이 내부적으로 새로 생성한다.
 */
export function relayOpenToTop(id: string, popupType: string, props: Record<string, unknown>): Promise<unknown> {
  ensureRelayListenerAttached();

  return new Promise((resolve, reject) => {
    pendingRelays.set(id, { resolve, reject });
    window.top?.postMessage(
      { channel: POPUP_BRIDGE_CHANNEL, type: 'relayOpen', id, popupType, props },
      window.location.origin
    );
  });
}
...
**Step 2: `popupApi.ts`의 `open()`이 iframe이면 relay하도록 수정**

상단 import에 추가:

```ts
import { isInIframe, relayOpenToTop } from './popupBridge';
```

`open()` 함수 전체를 아래로 교체 (기존 타임아웃/콜백 래핑 로직은 그대로 두고, "5. Redux에 dispatch" 단계만 분기):
  ...
  return new Promise<T>((resolve, reject) => {
    ...
    if (timeout > 0) {
      timeoutId = setTimeout(() => {
        if (!isSettled) {
          isSettled = true;
          // relay 경로는 로컬 store에 애초에 addPopup을 안 했으므로 로컬 removePopup 불필요.
          // (top에 열린 실제 팝업까지 취소하는 건 이번 범위에서 다루지 않음 - 알려진 제약 참고)
          if (!inIframe) {
    ...
    if (inIframe) {
      // 로컬 store에는 아무것도 추가하지 않는다 - window.top의 popup.open()이
      // 자기 store에 진짜 팝업을 추가하고, 그 결과만 relay로 돌려받는다.
      relayOpenToTop(id, popupType, (props ?? {}) as Record<string, unknown>)
        .then((result) => wrappedCallbacks.resolve(result as T))
        .catch((error: unknown) =>
          wrappedCallbacks.reject(error instanceof Error ? error : new Error(String(error)))
        );
      return;
    }
...
**Step 3: top 전용 relay 리스너 `PopupRelayHost` 작성**
...
import { isInIframe, isPopupBridgeMessage, POPUP_BRIDGE_CHANNEL } from '@/shared/utils/popup/popupBridge';

/**
 * PopupRelayHost: 최상위 window에서만 relayOpen 메시지를 듣고,
 * 자기 자신의 popup.open()을 그대로 재호출해서 진짜 DialogRoot가 렌더링하게 한다.
 *
 * @description
 * iframe(routed 팝업) 안에서는 isInIframe()이 true라 아무것도 하지 않는다 -
 * relay는 항상 window.top으로 한 홉에 직행하므로 중간 단계는 리스닝할 필요가 없다.
 */
export function PopupRelayHost() {
  useEffect(() => {
    ...
    function handleMessage(event: MessageEvent) {
      ...
      const data = event.data;
      if (!isPopupBridgeMessage(data) || data.type !== 'relayOpen') return;

      const source = event.source as Window | null;
      if (!source) return;

      popup
        .open(data.popupType, data.props)
        .then((result) => {
          source.postMessage(
            { channel: POPUP_BRIDGE_CHANNEL, type: 'relayResolve', id: data.id, result },
            window.location.origin
          );
        })
        .catch((error: unknown) => {
...
**Step 4: `layout.tsx`에 마운트**

`src/app/layout.tsx` 상단 import에 추가:

```tsx
import { PopupRelayHost } from '@/shared/components/popups/PopupRelayHost';
```
...
```bash
git add src/shared/utils/popup/popupBridge.ts \
        src/shared/utils/popup/popupApi.ts \
        src/shared/components/popups/PopupRelayHost.tsx \
        src/app/layout.tsx
git commit -m "feat: relay popup.open() calls from routed iframes to window.top to avoid clipping"
```
...
- Modify: `src/features/pub/shared/components/popups/Ltpz999Content.tsx` (relay 검증용 임시 버튼 — 검증 후 되돌림)

**Step 1: 임시 트리거 추가**

`PopupTestSection.tsx` 컴포넌트 상단 근처에 추가:
...
**Step 3: relay(서브 팝업 클리핑 해결) 검증**

`Ltpz999Content.tsx`의 "닫기" 버튼 근처에 임시로 추가:
...
<Button
  ...
  size={'xl'}
  color={'gray-light'}
  onClick={() => popup.confirm({ title: '서브 팝업', message: 'relay 테스트' })}
>
  서브 팝업 열기 (relay 테스트)
</Button>
```

(`import { popup } from '@/shared/utils/popup';` 추가 필요)

routed popup(iframe) 안에서 이 버튼 클릭 후:
1. 확인 다이얼로그가 **iframe 박스 안에 눌려 담기지 않고** 화면 전체 기준으로 정상 크기/중앙 정렬로 뜨는지 확인.
2. DevTools Elements에서 이 확인 다이얼로그가 **top 페이지의 `document.body`** 밑에 렌더링됐는지 확인 (iframe의 `#document` 안이 아님).
3. 딤(overlay)이 routed 팝업의 `DialogContent` chrome과 새 확인 다이얼로그 둘 다에 대해 순서대로(가장 위 팝업만 딤 표시) 올바르게 적용되는지 확인.
4. "확인"/"취소" 클릭 시 iframe 콘솔에서 `popup.confirm(...)`이 반환하는 Promise가 올바른 boolean으로 resolve되는지 확인 (`.then(console.log)`로 확인).

**Step 4: 트리거 되돌리기 또는 유지**

이번 검증용으로만 넣은 거라면 확인 후 `PopupTestSection.tsx`와 `Ltpz999Content.tsx`를 원래대로 되돌린다. `PopupTestSection.tsx` 쪽 트리거를 계속 남겨둘 거라면 정식 샘플로 커밋하되, `Ltpz999Content.tsx`의 relay 테스트 버튼은 실제 업무 화면이 아니므로 반드시 되돌린다.

**Step 5: 커밋 (PopupTestSection.tsx 트리거를 유지하는 경우)**
...
- ~~iframe 안에서 열리는 하위 팝업이 그 iframe 박스 크기로 클리핑되는 문제~~ → **Task 7에서 해결됨.** `popup.open()`이 `isInIframe()`이면 `window.top`으로 relay하고, top에 상시 마운트된 `PopupRelayHost`가 자기 자신의 `popup.open()`을 재호출해서 진짜 최상위 `DialogRoot`가 렌더링하게 한다. 부수 효과로 `dialogOverlayRegistry.ts`(모듈 싱글턴이라 iframe마다 별도 인스턴스였던 것)도 top 하나로 통일되어 딤(overlay) 순서가 자동으로 맞는다.
- **동적으로 등록된(`registerDialog()`) feature 팝업을 routed 팝업 안에서 서브 팝업으로 여는 경우는 여전히 안 됨.** `confirm`/`alert`/`LTPZ995`처럼 `popupRegistry.ts`에 **정적으로** 등록된 타입은 모든 window(top이든 iframe이든)에 동일하게 존재하므로 relay 후 top에서 바로 찾아진다. 하지만 어떤 팝업이 마운트 시점에 `registerDialog('foo/bar', loader)`를 호출해서 등록한 타입은 그 등록이 일어난 window(예: iframe)의 로컬 `dialogRegistry`에만 존재한다 — relay로 top에 넘어가면 top의 `dialogRegistry`엔 없어서 "Dialog not found" 에러가 난다. 지금 레퍼런스 대상인 `Ltpz999`는 내부에서 서브 팝업을 열지 않으므로 이 제약에 안 걸린다. 나머지 팝업을 마이그레이션하기 전에, 대상 팝업이 자기 내부에서 동적 등록된 타입을 `popup.open()`으로 여는지부터 확인해서 이 제약에 걸리는지 점검할 것 — 걸린다면 그 타입을 top 레벨에서도 미리(정적으로) 등록해두는 식으로 개별 대응.
- **relay 도중 타임아웃이 발생하면, top에 이미 열린 실제 팝업까지 취소되진 않는다.** `popup.open(..., { timeout })`을 iframe 안에서 호출했는데 타임아웃이 먼저 발생하면 호출자의 Promise는 reject되지만, top에서는 그 팝업이 여전히 열려있는 채로 남는다(사용자가 직접 닫아야 함). timeout을 쓰는 routed 팝업 서브 호출이 실제로 필요해지면, relay 취소 메시지(`relayCancel` 같은)를 추가해서 top의 팝업도 함께 정리하도록 확장하면 된다 — 지금은 그런 사용처가 없어서 미리 만들지 않음.
...
- `src/app/layout.tsx`의 구조 변경이나 route group 재구성 — Task 7에서 `<PopupRelayHost />` 한 줄을 `<DialogRoot />` 옆에 추가하는 것 외에는 손대지 않는다. iframe 경계 자체가 window/Redux 격리를 제공하므로 레이아웃 재구성은 여전히 불필요함.
