...
interface ProductData {
  id: number;
  productName: string;
  dep2: ProductDataDep2[];
}

interface ProductInformationTableDep2Props {
...
const ProductNameRenderer = (props: ICellRendererParams<ProductDataDep2>) => {
...
export function ProductInformationTableDep2({ product, onSelectDep2 }: ProductInformationTableDep2Props) {
  const columnDefs: ColDef<ProductDataDep2>[] = useMemo(
    () => [
      {
        headerName: '종구분',
        field: 'name',
  ...
  const handleSelectionChanged = (event: { api: GridApi<ProductDataDep2> }) => {
