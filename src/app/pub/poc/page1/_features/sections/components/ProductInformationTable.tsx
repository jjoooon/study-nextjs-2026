...
import { Checkbox, Badge, Input, Button } from '@/shared/components/uiux';
...
interface ProductInformationTableProps {
  data: ProductData[];
  selectedProductId: number | null; // 향후 프로그래매틱 선택을 위해 유지
...
const ProductNameRenderer = (props: ICellRendererParams<ProductData>) => {
...
const FavoriteRenderer = (props: ICellRendererParams<ProductData>) => {
...
export function ProductInformationTable({
  data,
  ...
  const handleSelectionChanged = (event: { api: GridApi<ProductData> }) => {
    const selectedNodes = event.api.getSelectedNodes();
    if (selectedNodes.length > 0) {
      const selectedData = selectedNodes[0].data;
      if (selectedData) {
        onSelectProduct(selectedData.id);
      }
    }
  };
