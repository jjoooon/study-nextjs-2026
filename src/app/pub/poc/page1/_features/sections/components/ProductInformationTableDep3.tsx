...
// Types
interface ProductDataDep3 {
  plan: string;
  name: string;
  id?: string | null;
}

interface ProductInformationTableDep3Props {
  dep3Data: ProductDataDep3[];
}
...
const ProductNameRenderer = (props: ICellRendererParams<ProductDataDep3>) => {
...
export function ProductInformationTableDep3({ dep3Data }: ProductInformationTableDep3Props) {
  const { active, setActive, visibleTabs } = useTabs(initialTabs);

  const columnDefs: ColDef<ProductDataDep3>[] = useMemo(
    () => [
      {
        headerName: '플랜명',
        field: 'name',
