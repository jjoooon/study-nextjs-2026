...
import { SHARED_MOCK_DATA } from '../constants/main';
import { ProductInformationTable } from './components/ProductInformationTable';
import { ProductInformationTableDep2 } from './components/ProductInformationTableDep2';
import { ProductInformationTableDep3 } from './components/ProductInformationTableDep3';
...
// 상품 정보 섹션
export function ProductInformationSection({ filterCategory, filterAge }: ProductInformationSectionProps) {
  ...
  const filteredData = useMemo(() => {
    ...
    return SHARED_MOCK_DATA.filter((product) => {
      // 카테고리 필터
      if (filterCategory !== '0') {
      ...
      <ResizablePanel defaultSize={30} minSize={'30%'}>
        <ResizablePanelGroup orientation="vertical">
          ...
          <ResizablePanel defaultSize={58} minSize={'30%'} className="resizablePanel-line">
            <ProductInformationTableDep3 dep3Data={selectedDep2?.dep3 || []} />
          </ResizablePanel>
        </ResizablePanelGroup>
      </ResizablePanel>
    </ResizablePanelGroup>
