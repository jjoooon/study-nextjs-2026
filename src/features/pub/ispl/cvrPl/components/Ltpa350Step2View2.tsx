...
interface DummyDataType {
  ...
  num: number | null;
  field1?: string | number | boolean | string[] | number[];
  field2?: string | number | boolean | string[] | number[];
  field3?: string | number | boolean | string[] | number[];
  ...
  field9?: string | number | boolean | string[] | number[];
  field10?: {
    title: string;
    description: string;
    info: string[];
...
export function Ltpa350Step2View2({ onSelectPlan, isWidthExpanded = false, setIsWidthExpanded }: Ltpa350Step2Props) {
  ...
  const columnDefs: ColDef<AgGridRow>[] = useMemo(
    () => [
      ...
      {
        headerComponent: () => (
          <Gcol className="w-full" placement={'cc'} gap={0}>
            가입금액<span className="text-[1.1rem] leading-t">(만원)</span>
          </Gcol>
        ),
        field: 'field3',
      ...
      },
      {
        ...
        children: [
          {
            headerName: '출생전',
            valueGetter: (params: ValueGetterParams<AgGridRow>) =>
              Array.isArray(params.data?.expiry) ? params.data.expiry[0] : params.data?.expiry,
            width: attributeColumnWidth[7],
            cellClassRules: editableCellClassRules,
            cellClass: (params: CellClassParams<AgGridRow>) => {
              const base = 'px-[0.2rem]! tracking-tighter';
              const isEditable = Array.isArray(params.data?.isExpiryEditable)
                ? params.data.isExpiryEditable[0]
                : params.data?.isExpiryEditable;
              return isEditable === true ? base : `${base} no-edited`;
            },
            editable: (params: EditableCallbackParams<AgGridRow>) => {
              return Array.isArray(params.data?.isExpiryEditable)
                ? params.data.isExpiryEditable[0] === true
                : params.data?.isExpiryEditable === true;
            },
          ...
          },
          {
            ...
            cellClass: (params: CellClassParams<AgGridRow>) => {
              const base = 'px-[0.2rem]! tracking-tighter';
              return params.data?.isEditedField5b === true ? base : `${base} no-edited`;
            },
            editable: (params: EditableCallbackParams) => {
              return params.data?.isEditedField5b === true;
            },
            cellEditor: 'agSelectCellEditor',
            cellEditorParams: {
      ...
      },
      {
        headerName: '예상UW',
        headerComponent: () => (
          <Gcol className="w-full leading-tight" placement={'cc'} gap={0}>
