...
//
// 수정 내역:
// 1. AccordionStyle 타입 추가 ("default" | "box" | "line" | "minimal")
...
function Accordion({
  ...
  return (
    <AccordionContext.Provider value={accostyle}>
      <AccordionPrimitive.Root data-slot="accordion" className={cn(accordionStyles[accostyle], className)} {...props} />
...
function AccordionItem({ className, ...props }: React.ComponentProps<typeof AccordionPrimitive.Item>) {
  ...
  return (
    <AccordionPrimitive.Item data-slot="accordion-item" className={cn(itemStyles[accostyle], className)} {...props} />
  );
}
  ...
  return (
    <AccordionPrimitive.Header className="flex relative min-h-[5.75rem]">
      ...
      <AccordionPrimitive.Trigger
        ...
        aria-label=""
        className={cn('absolute inset-0 justify-end!', triggerStyles[accostyle], className)}
        {...props}
      >
