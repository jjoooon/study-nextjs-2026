              ...
              return (
                <button
                  ...
                  className={`flex flex-col w-full gap-1 items-center justify-center rounded-tr-[0.8rem] py-[0.6rem] hover:bg-[#FFEFBF] ${
                    isActive ? 'bg-[linear-gradient(344deg,_#FF5C2E_-17.78%,_#FF8D02_88.79%)] text-white' : 'bg-transparent text-[var(--color-gray-70)]'
                  }`}
                ...
                >
                  ...
                  >
                    {isComplete ? <CheckBoldIcon /> : isActive ? <ProcessActiveIcon className="animate-spin [animation-duration:4s]" /> : item.step}
                  </b>
                  <Typo className={`px-2 text-[1.1rem] leading-[1.3rem] text-center ${isActive ? 'text-white' : 'text-[var(--color-gray-70)]'}`}>
                    {item.label}
                  </Typo>
                </button>
              );
