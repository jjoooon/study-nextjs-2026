...
export const ViewMode = ({ state = true, label, onChange }: ViewModeProps) => {
  return (
    <div className="flex items-center space-x-2">
      ...
      <label
        htmlFor="docType"
        className="relative h-[2.8rem] rounded-full border border-[var(--color-gray-15)] flex justify-center items-center gap-[1.1rem] text-[var(--color-secondary-70)] text-[1.3rem] peer-checked:[&>.peer-2]:text-[var(--color-gray-0)] peer-checked:[&>.peer-2]:font-bold peer-checked:[&>.peer-1]:text-[var(--color-gray-50)] peer-checked:[&>.peer-1]:text-normal peer-checked:[&>b]:left-[2.9rem] cursor-pointer select-none"
      >
        <span className="peer-1 block relative z-1 text-[1.1rem] text-[var(--color-gray-0)] font-bold tracking-[-0.13rem] white-space-nowrap">
          {label?.[0] ?? '간편'}
        </span>
        <span className="peer-2 block relative z-1 text-[1.1rem] text-[var(--color-gray-50)] tracking-[-0.13rem] white-space-nowrap">
          {label?.[1] ?? '상세'}
        </span>
