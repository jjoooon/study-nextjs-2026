 ...
 * @param windowFeatures - 창 속성 문자열 (예: `"width=800,height=600,noopener,noreferrer"`)
 * @param onBlocked - 팝업이 차단된 경우 호출되는 콜백. 생략 시 콘솔 경고 출력
 * @returns 열린 창의 `WindowProxy`, 차단되었거나 실패한 경우 `null`
 */
export const openBrowser = (url: string, target?: string, windowFeatures?: string, onBlocked?: () => void) => {
